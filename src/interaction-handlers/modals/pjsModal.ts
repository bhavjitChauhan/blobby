...
export class ModalHandler extends InteractionHandler {
	...
	public override async parse(interaction: ModalSubmitInteraction) {
		...
		options.width = clamp(parseInt(options.width ?? '400'), 1, 5e3);
		options.height = clamp(parseInt(options.height ?? '400'), 1, 5e3);
		options.delay = clamp(parseFloat(options.delay ?? '0'), 0, Time.Minute);
		options.canvas = options.canvas ?? true;
		options.loopProtector = options.loopProtector ?? true;

		return this.some({ message, code, ...(options as EvalOptions) });
	}
	public async run(interaction: ModalSubmitInteraction, data: InteractionHandler.ParseResult<this>) {
		const { message, code, ...options } = data;
